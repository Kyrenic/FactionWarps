...
class FactionWarpDeleteCommand(private val plugin: FactionWarps) : CommandExecutor, TabCompleter {
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        ...
        if (!sender.hasPermission("fw.warp.delete")) {
            sender.sendMessage("${ChatColor.RED}${plugin.language["NoPermission"]}")
            return true
        }
        // Command needs one argument.
        if (args.size !in 1..1) {
            sender.sendMessage("${ChatColor.RED}${plugin.language["CommandWarpDeleteUsage"]}")
            return true
        }
        ...
        if (role == null || !role.hasPermission(senderFaction, factionPermissions.warpDelete)) {
        ...
        warpService.deleteWarp(warp)
        sender.sendMessage("${ChatColor.GREEN}${plugin.language["WarpDeleted", warpName]}")
        return true
    }
